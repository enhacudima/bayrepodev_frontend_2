<template>
    ...
    <a-table :columns="columns" :row-key="record => record.id" :data-source="tableData" :scroll="{ x: 1500, y: 600 }" :pagination="pagination"
        ...
        <template #bodyCell="{ column, text, record }">
                    ...
                    <a-row v-for="(bypass,key) in record.bypass" :key="key">
                        <span v-if="record.bypass">
                            ...
                            <a-tooltip color="blue" v-if="bypass.file">
                                ...
                                <a :href="$baseUrl + 'storage/' + bypass.file.path + bypass.file.name" target="_blank">
                                    <PaperClipOutlined />
                                </a>
                            </a-tooltip>
                        </span>
            ...
            <template v-if="column.dataIndex === 'type'">
                    <span>
                        <a-tag
                            :color="record.type === 'Risk Categorization' ? 'green' : 'geekblue'">
                            {{ record.type }}
                        </a-tag>
                    </span>
            </template>
    ...
    <a-modal v-model:open="visible" :title="detailesData.id + ' - ' + detailesData.employee_number + ' - ' + detailesData.surname + ', ' + detailesData.client_first_names" :footer="null" width="1000px">
        <a-tabs v-model:activeKey="activeKey">
            ...
            <a-tab-pane key="1"  force-render>
                ...
                <tabBox :tabBoxRef="{ 'NUIT': detailesData.employee_number, 'Date_Of_Birth': detailesData.date_of_birth }" />
            </a-tab-pane>
        </a-tabs>
    </a-modal>
    <a-modal v-model:visible="visibleAMl"
        :title="detailesData.id + ' - ' + detailesData.employee_number + ' - ' + detailesData.surname +', '+detailesData.client_first_names" :footer="null">
        <a-form ref="formRef" :model="form" :rules="rules" layout="vertical" @finish="onFinish">
            ...
            <a-form-item name="fileList" label="Upload" extra="Support document">
                <a-upload v-model:file-list="form.fileList" list-type="picture" :max-count="1"
                    :action="$baseUrlAPI + '/v1/upload'" method="post"  :data="{'my_token':detailesData.employee_number }">
                    <a-button>
...
import { CheckOutlined,  EyeOutlined, CloseCircleOutlined, UploadOutlined,  CodeSandboxOutlined, FileSearchOutlined, PaperClipOutlined } from '@ant-design/icons-vue';
...
export default defineComponent({
    ...
    setup() {
        ...
        const fetch = (params = {}) => {
            ...
            axios.get('/v1/screening-table-data?' + u)
        ...
        function send(values) {
            axios.post("v1/screening-aml-submit-doc", values)
                .then(response => {
                    response
        ...
        const onAMLBypass = (data) => {
            axios.post('v1/screening-aml-bypass', data)
                .then(response => {
                    response
                    eventBus.$emit("table-list-Changed")
                }
        ...
        const onBlackListBypass = (data) => {
            axios.post('v1/screening-black-list-bypass', data)
                .then(response => {
                    response
                    eventBus.$emit("table-list-Changed")
                }
        ...
        const onBlackList = (data) => {
            axios.post('v1/screening-black-list', data)
                .then(response => {
                    response
                    eventBus.$emit("table-list-Changed")
                    }
        ...
        const onAML = (data) => {
            axios.post('v1/screening-aml', data)
                .then(response => {
                    response
                    eventBus.$emit("table-list-Changed")
                    }
        ...
        const onAMLUnblock = (data) => {
            axios.post('v1/screening-aml-unblock', data)
                .then(response => {
                    response
                    eventBus.$emit("table-list-Changed")
                }
        ...
        const onBlackListLUnblock = (data) => {
            axios.post('v1/screening-black-list-unblock', data)
                .then(
                    response => {
                        response
                    eventBus.$emit("table-list-Changed")
        ...
        const onSearch = (() => {
            ...
            loading.value = true
            axios.get('v1/screening-table-data/search/' + filter)
                .then(response => {
