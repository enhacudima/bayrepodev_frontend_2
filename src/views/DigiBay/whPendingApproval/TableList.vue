<template>
    <a-input-search v-model:value="searchValue" placeholder="Search here" @search="onSearch" :loading="loading" enter-button :style="{ marginBottom: '24px', width: '500px' }" />
    <a-table :columns="columns" :row-key="record => record.id" :data-source="tableData" :loading="loading" :scroll="{ x: 1500, y: 600 }" @change="onChange">
        <template #bodyCell="{ column, text, record }">
            ...
            <template v-if="column.dataIndex === 'employer_type'">
                <span>
                    <a-tag
                        :color="record.employer_type === 'VG' ? 'geekblue' : 'green'">
            ...
            <template v-if="column.dataIndex === 'Reference'">
                <span>
                    <a-tag v-if="record.Reference === 'Missing Reference' || record.Reference === 'Check Reference'" :color="'#eb2f96'">
                        {{ record.Reference }}
                    </a-tag>
                    <a-tag v-else
                        :color="'green'">
            ...
            <template v-if="column.dataIndex === 'ready_approval'">
                ...
                <span v-if="record.ready_approval == 1 || record.ready_approval == 2">
                    <a-popover title="Reason" >
                        ...
                        <check-circle-outlined :style="{ color: '#52c41a'}" v-if="record.ready_approval == 1" />
                        <close-circle-outlined :style="{ color: '#eb2f96'}" v-if="record.ready_approval == 2"/>
                    </a-popover>

                </span>
                <close-outlined :style="{ color: '#eb2f96'}"  v-if="record.ready_approval == 0 || record.ready_approval == null || record.ready_approval == ''"/>
            </template>
            <template v-else-if="column.dataIndex === 'affordability'">
                <span v-if="record.affordability !== null">
                        <a-tooltip placement="top" :title="record.affordability">
                            <alert-outlined :style="{ color: '#eb2f96'}" v-if="record.affordability == 'Impossible to verify'" />
                            <flag-outlined :style="{ color: '#52c41a'}" v-else-if="record.affordability == 'Good for business'" />
                            <caret-down-outlined :style="{ color: '#eb2f96'}" v-else-if="record.affordability == 'Below the maximum capacity'" />
                            <span v-if="record.affordability != 'Impossible to verify'" >
                                <a-divider type="vertical" /> {{ new Intl.NumberFormat().format(record.affordability_percent) }}%
                            </span>
                        </a-tooltip>
                </span>
            ...
            <template v-else-if="column.dataIndex === 'check_telephone'">
                <check-outlined :style="{ color: '#52c41a'}"
                    v-if="record.check_telephone == 1" />
                <span v-else>
                    <a-popover title="Phones">
                        <template #content>
                            <p  v-for="(telephone,index) in record.telephones" :key="index"><b>Name: </b>{{ telephone.name }}, {{ telephone.table }}, {{ telephone.phone }} </p>
                        </template>
                        <div class="demo-dropdown-wrap" v-if="record.check_telephone == 0" >
                            <a-dropdown-button type="primary">
                                ...
                                <template #overlay>
                                    <a-menu>
                                        <a-menu-item key="1">
                                            <a-popconfirm title="Sure to bypass this record?" @confirm="onTelephoneBypass(record)">
                                                <a-button type="primary" small>Accept Risk</a-button>
                                            </a-popconfirm>
                                        </a-menu-item>
                                        <a-menu-item key="2">
                                            <a-popconfirm title="Sure to set high risk this record?" @confirm="onTelephone(record)">
                        ...
                        <close-circle-outlined :style="{ color: '#eb2f96'}" v-if="record.check_telephone == 2" />
            ...
            <template v-else-if="column.dataIndex === 'Is_OTP_Validated'">
                <check-outlined :style="{ color: '#52c41a'}"
                    v-if="record.Is_OTP_Validated == 1" />
                <span v-else>
                    <a-popconfirm title="Sure to bypass this record?" @confirm="onOTPBypass(record)">
                        <a-button type="primary" small>Accept OTP</a-button>
                    </a-popconfirm>
                </span>
            </template><template v-else-if="column.dataIndex === 'cdr'">
                <a-popconfirm title="Sure to set not accept this record?" v-if="record.cdr == 1"
                    @confirm="onCdr(record, 'wh-pending-approval/cdr')">
                    <check-outlined :style="{ color: '#52c41a'}" />
                </a-popconfirm>
                <a-popconfirm title="Sure to unblock this record?" v-else-if="record.cdr == 2"
                ...
                <div class="demo-dropdown-wrap" v-else>
                    <a-dropdown-button type="primary">
                        ...
                        <template #overlay>
                            <a-menu>
                                <a-menu-item key="2">
                                    <a-popconfirm title="Sure to bypass this record?" @confirm="onCdr(record, 'wh-pending-approval/cdr-bypass')">
                                        <a-button type="primary" small>Accept</a-button>
                                    </a-popconfirm>
                                </a-menu-item>
                                <a-menu-item key="3">
                                    <a-popconfirm title="Sure to set not accept this record?" @confirm="onCdr(record, 'wh-pending-approval/cdr')">
            ...
            <template v-else-if="column.dataIndex === 'verificacao_nib'">
                ...
                <span v-else>
                    <a-popconfirm :title="'Sure to bypass this record. ('+ record.verificacao_msg +')?'" @confirm="onQaBypass(record)">
            ...
            <template v-else-if="column.dataIndex === 'aml'">
                ...
                <a-popconfirm title="Sure to unblock this record?" v-if="record.aml==2" @confirm="onAMLUnblock(record)">
                    <close-circle-outlined :style="{ color: '#eb2f96'}"  />
                </a-popconfirm>
            ...
            <template v-else-if="column.dataIndex === 'black_list'">
                <div class="demo-dropdown-wrap" v-if="record.black_list==1">
                    <a-dropdown-button type="primary">
                        ...
                        <template #overlay>
                            <a-menu>
                                <a-menu-item key="2">
                                    <a-popconfirm title="Sure to bypass this record?" @confirm="onBlackListBypass(record)">
                                        <a-button type="primary" small>Accept Risk</a-button>
                                    </a-popconfirm>
                                </a-menu-item>
                                <a-menu-item key="3">
                ...
                <a-popconfirm title="Sure to unblock this record?" v-if="record.black_list==2" @confirm="onBlackListLUnblock(record)">
                    <close-circle-outlined :style="{ color: '#eb2f96'}" />
                </a-popconfirm>
    ...
    <a-modal v-model:open="visible" :title="detailesData.Application_ID + ' - ' + detailesData.NUIT + ' - ' + detailesData.Surname + ', ' + detailesData.First_Name" :footer="null" width="1000px">
    ...
    <a-modal v-model:visible="visibleAMl"
        :title="detailesData.Application_ID + ' - ' + detailesData.NUIT + ' - ' + detailesData.Surname +', '+detailesData.First_Name" :footer="null">
        <a-form ref="formRef" :model="form" :rules="rules" layout="vertical" @finish="onFinish">
            ...
            <a-form-item name="fileList" label="Upload" extra="Support document">
                <a-upload v-model:file-list="form.fileList" list-type="picture" :max-count="1"
                    :action="$baseUrlAPI + '/v1/upload'" method="post" :headers="headers" :data="{'my_token':detailesData.Application_ID, '_token':headers['X-CSRF-TOKEN']}">
                    <a-button>
...
import { CheckOutlined, CloseOutlined, EyeOutlined, CloseCircleOutlined, UploadOutlined, LoadingOutlined,  CheckCircleOutlined, FlagOutlined, CaretDownOutlined, AlertOutlined, FileSearchOutlined, CodeSandboxOutlined } from '@ant-design/icons-vue';
    ...
    setup() {
        ...
        const columns = [
            ...
            },
            {
                ...
                onFilter: (value, record) => {
                    if (record.check_telephone == 0 || record.check_telephone == 1 || record.check_telephone == 2) {
        ...
        function send(values) {
            axios.post("v1/wh-pending-approval/aml-submit-doc", values)
                .then(
                    response => {
                        message.success({
                            content: () => response.data.message+' The change will be reflected in the next update.',
                            class: 'custom-class',
                            style: {
                                marginTop: '10vh',
                            },
        ...
        const onQaBypass = (data) => {
            axios.post('v1/wh-pending-approval/qa-bypass', data)
                .then(
        ...
        const onOTPBypass = (data) => {
            axios.post('v1/wh-pending-approval/otp-bypass', data)
                .then(
        ...
        const onAMLBypass = (data) => {
            axios.post('v1/wh-pending-approval/aml-bypass', data)
                .then(
                    eventBus.$emit("table-list-Changed"),
                    message.success({
                        content: () => 'The change will be reflected in the next update.',
        ...
        const onBlackListBypass = (data) => {
            axios.post('v1/wh-pending-approval/black-list-bypass', data)
                .then(
                    eventBus.$emit("table-list-Changed"),
                    message.success({
                        content: () => 'The change will be reflected in the next update.',
        ...
        const onTelephone = (data) => {
            axios.post('v1/wh-pending-approval/telephone', data)
                .then(
                    eventBus.$emit("table-list-Changed"),
                    message.success({
                        content: () => 'The change will be reflected in the next update.',
        ...
        const onTelephoneBypass = (data) => {
            axios.post('v1/wh-pending-approval/telephone-bypass', data)
                .then(
                    eventBus.$emit("table-list-Changed"),
                    message.success({
                        content: () => 'The change will be reflected in the next update.',
        ...
        const onBlackList = (data) => {
            axios.post('v1/wh-pending-approval/black-list', data)
                .then(
                    eventBus.$emit("table-list-Changed"),
                    message.success({
                        content: () => 'The change will be reflected in the next update.',
        ...
        const onAML = (data) => {
            axios.post('v1/wh-pending-approval/aml', data)
                .then(
                    eventBus.$emit("table-list-Changed"),
                    message.success({
                        content: () => 'The change will be reflected in the next update.',
        ...
        const onAMLUnblock = (data) => {
            axios.post('v1/wh-pending-approval/aml-unblock', data)
                .then(
                    eventBus.$emit("table-list-Changed"),
                    message.success({
                        content: () => 'The change will be reflected in the next update.',
        ...
        const onBlackListLUnblock = (data) => {
            axios.post('v1/wh-pending-approval/black-list-unblock', data)
                .then(
                    eventBus.$emit("table-list-Changed"),
                    message.success({
                        content: () => 'The change will be reflected in the next update.',
        ...
        const onSearch = (() => {
            ...
            loading.value = true
            axios.get('v1/wh-pending-approval/search/' + filter)
                .then(response => {
