<template>
    <div>
        <a-modal v-model:open="visible" width="1000px"  title="Create a new Role" :footer="null"
            >
            <a-form ref="formRef" :model="formState" layout="vertical" name="form_in_modal" @finish="onFinish">
                ...
                <a-form-item name="guard_name" label="Guard name" :rules="[{ required: true, message: 'Please input the name of guard name!' }]">
...
export default defineComponent({
    setup() {
        ...
        function send(values) {
            axios.post("/v1/roles", values)
                .then(() => {
                    Modal.success({
                        title: 'Action',
                        content: h('div', {}, [
                            h('p', 'Role Created Successfully'),
                        ]),
                    });
                    visible.value =false
                    formRef.value.resetFields()
