<template>
    <a-modal v-model:open="visible"  width="1000px" :title="'Edit User: '+form.name" :footer="null"
            >
    <a-form ref="formRef" :model="form" v-bind="layout" name="nest-messages" :validate-messages="validateMessages"
        ...
        <a-collapse v-model:activeKey="activeKey" :style="{ marginBottom: '24px' }">
            <a-collapse-panel key="1" header="User Info">
                ...
                <a-form-item name="expire" label="Expire Type" has-feedback
                    :rules="[{ required: true, message: 'Please select expire type!' }]">
                    <a-select v-model:value="form.expire" placeholder="Expire Type">
                        <a-select-option value="default">default</a-select-option>
...
export default defineComponent({
    setup() {
        ...
        function send(values) {
            axios.put("/v1/users/"+id.value, values)
                .then(() => { 
                    Modal.success({
                        title: 'User Update',
                        content: h('div', {}, [
                            h('p', 'Successfully Update User.'),
                        ]),
                    });
                    visible.value = false
