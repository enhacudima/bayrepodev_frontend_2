<template>
    <a-button type="primary" :style="{ marginBottom: '24px' }" @click="showNewRole()">New Fee</a-button>
    <a-table :columns="columns"  :row-key="record => record.id" :data-source="dataSource" :pagination="pagination"
        ...
        <template #bodyCell="{ column, text, record }">
            ...
            <template v-else-if="column.dataIndex === 'action'">
                <span>
                    <a-popconfirm title="Sure to delete this fee?" @confirm="onDelete(record.id)">
                        <a>Delete</a>
                    </a-popconfirm>
                </span>
           </template>
...
export default defineComponent({
    ...
    setup() {
        ...
        }, 
        {
            ...
            sorter: true,
            filters: [{
                text: '200 (New Account)',
                value: '200',
            }, 
            {
                text: '201 (New Group Client)',
                value: '201',
            },
            {
                text: '202 (New Saving)',
