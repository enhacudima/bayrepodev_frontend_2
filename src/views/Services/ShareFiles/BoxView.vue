        ...
        <a-row :gutter="16">
          <a-col :span="8" v-for="(file, index) in filesClient" :key="index">
            ...
            >
              <template #cover>
                <a-image
                  style="width: 510px; max-width: 510px; height: 200px; max-height: 200px"
              ...
              <a-card-meta :title="file.original_name">
                <template #description>
                  <a-tooltip color="blue">
                    <template #title
                      >Name: {{ file.original_name }}, Date:
                      {{ dataTimeFormat(file.updated_at) }},User:
...
export default defineComponent({
  ...
  setup() {
    ...
    const onSearch = () => {
      ...
      state.fetching = true
      axios
        .get('v1/box-search-get/' + uuid.value)
        .then((response) => {
    ...
    const onBlock = (record) => {
      axios.get('/v1/shared-files/block/' + record.my_token).then(function () {
        fetch()
      })
    }
