<template>
    <a-modal v-model:open="visible"  width="1000px" :title="form.name" :footer="null"
            >
    <a-form ref="formRef" :model="form" v-bind="layout" name="nest-messages" :validate-messages="validateMessages"
            ...
            <a-form-item :wrapper-col="{ ...layout.wrapperCol, offset: 6 }">
                <a-button type="primary" html-type="submit">{{ $t('update')}}</a-button>
...
export default defineComponent({
    setup() {
        ...
        function send(values) {
            axios.put("/v1/smsboundtemplate/" + form.id, values)
                .then(response => { 
                    Modal.success({
                        title: 'Update SMS Template Saved',
                        content: h('div', {}, [
