<template>
    <a-card title="Loan Simulator" :bordered="true" :style="{ width: '100%', marginTop:'10px'}">
        ...
        -->
            ...
            <a-form-item  label="Amount" name="amount" :rules="[{ required: true, message: 'Please select loan amount!' }]" has-feedback :style="{ width: '100%', marginTop:'24px'}">
                <a-input type="number" @wheel="$event.target.blur()" v-model:value="form.amount" addon-before="MTN"
...
import dates from "../AmountPaymentPlan/LoansView.vue";
export default defineComponent({
    ...
    setup() {
        ...
        function send(values) {
            axios.post("/v1/loan-simulator-amount", values)
                .then(response => {
                    datesDate.value.loading = true
                    loanDetails.value = response.data
                    datesDate.value.dataSource = response.data.amortization_schedule
